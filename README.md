...
Full flow from zero to viewing an extracted knowledge graph.

### Step 1 — Deploy the VM

Run the **deploy** workflow from GitHub Actions (Actions → deploy → Run workflow). It takes ~20–30 minutes and sets up the VM, NVIDIA drivers, Ollama, Docker, Open WebUI, and Grafana Alloy end-to-end.

When it turns green, verify you can reach Open WebUI:
```
http://ollama-vm-1.<your-tailnet>.ts.net
```
Create your admin account on first visit.

### Step 2 — Pull a model

Run the **pull-model** workflow (Actions → pull-model → Run workflow):
- `model`: pick `qwen2.5:14b` (good balance of quality and speed on a T4)
- `context_size`: leave at `0` to use the default (20k)

This pulls the model and creates the `qwen2.5-14b-20k` variant (~9 GB, takes 5–10 min).

### Step 3 — Deploy extensions

Run the **deploy-extensions** workflow (Actions → deploy-extensions → Run workflow).

> Wait for pull-model to fully complete first — only one `az vm run-command` can run at a time per VM.

This installs:
- **Filebrowser** at `http://ollama-vm-1.<your-tailnet>.ts.net:8082` (login: `admin` / `admin`)
- **hyper-extract** + `nomic-embed-text` embedding model

### Step 4 — Upload a document

Open Filebrowser at `:8082`, navigate to `hyper-extract-input/`, and upload your file (PDF, Markdown, text).

### Step 5 — Extract knowledge

Run the **extract** workflow (Actions → extract → Run workflow):
- `file_path`: filename only, e.g. `paper.pdf`
- `template`: e.g. `general/concept_graph` for technical docs, `general/biography_graph` for narrative
- `model`: `qwen2.5-14b-20k`
- `lang`: `en`

> Run `he list template` on the VM to see all 46 templates.

Wait for the workflow to complete (can take minutes to hours depending on document size). Output appears in Filebrowser under `hyper-extract-output/<timestamp>-<filename>/`.

### Step 6 — View the knowledge graph

**Terminal 1** — start the OntoSight viewer on the VM:
```bash
...
**Terminal 2** — open an SSH tunnel to your machine:
```bash
ssh -N -L 8000:127.0.0.1:8000 azureuser@ollama-vm-1.<your-tailnet>.ts.net
```

Open **http://localhost:8000** in your browser. Keep Terminal 2 open while viewing; `Ctrl+C` to close the tunnel when done.
